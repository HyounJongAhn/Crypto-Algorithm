...
static void processChunk(SHA384_CTX* ctx)
{
    ...
    for (uint8_t i = 0; i < 16; ++i) {
        T1 = h + K[i] + ctx->w[i] + BSIG1_64(e) + CH64(e, f, g);
        T2 = BSIG0_64(a) + MAJ64(a, b, c);
        h = g;  g = f;  f = e;  e = d + T1;
        d = c;  c = b;  b = a;  a = T1 + T2;
    }
    for (uint8_t i = 16; i < 80; ++i) {
        uint64_t s0 = SSIG0_64(ctx->w[(i - 15) & 0x0F]);
        uint64_t s1 = SSIG1_64(ctx->w[(i - 2) & 0x0F]);
        ctx->w[i & 0x0F] = ctx->w[(i - 16) & 0x0F] + s0
            + ctx->w[(i - 7) & 0x0F] + s1;
        T1 = h + K[i] + ctx->w[i & 0x0F] + BSIG1_64(e) + CH64(e, f, g);
        T2 = BSIG0_64(a) + MAJ64(a, b, c);
        h = g;  g = f;  f = e;  e = d + T1;
        d = c;  c = b;  b = a;  a = T1 + T2;
    }
...
{
    uint64_t prev = ctx->lengthLow;
    ctx->lengthLow += ((uint64_t)len << 3);
    ctx->lengthHigh += ((uint64_t)len >> 61);
    if (ctx->lengthLow < prev) ctx->lengthHigh++;

    const uint8_t* p = (const uint8_t*)data;
    while (len) {
        size_t take = SHA384_BLOCK_SIZE - ctx->chunkSize;
        if (take > len) take = len;
        memcpy(((uint8_t*)ctx->w) + ctx->chunkSize, p, take);
        ctx->chunkSize += take;
        p += take;
        len -= take;
        if (ctx->chunkSize == SHA384_BLOCK_SIZE) {
            processChunk(ctx);
            ctx->chunkSize = 0;
        }
    }
...
void SHA384_Final(SHA384_CTX* ctx, uint8_t hash[SHA384_DIGEST_SIZE])
{
    ...
    if (ctx->chunkSize <= SHA384_BLOCK_SIZE - 17) {
        buf[ctx->chunkSize] = 0x80;
        memset(buf + ctx->chunkSize + 1, 0x00,
            SHA384_BLOCK_SIZE - 16 - (ctx->chunkSize + 1));
    }
    else {
        buf[ctx->chunkSize] = 0x80;
        memset(buf + ctx->chunkSize + 1, 0x00,
            SHA384_BLOCK_SIZE - (ctx->chunkSize + 1));
        processChunk(ctx);
        memset(buf, 0x00, SHA384_BLOCK_SIZE - 16);
    }

    ctx->w[14] = HTOBE64(ctx->lengthHigh);
    ctx->w[15] = HTOBE64(ctx->lengthLow);
    processChunk(ctx);

    for (uint8_t i = 0; i < 8; ++i)
        ctx->w[i] = HTOBE64(ctx->h[i]);
    memcpy(hash, ctx->w, SHA384_DIGEST_SIZE);
}

void SHA384_HMAC_Init(SHA384_CTX* ctx, const void* key, size_t keyLen)
{
    uint8_t K0[SHA384_BLOCK_SIZE] = { 0 };
    if (keyLen > SHA384_BLOCK_SIZE) {
        SHA384_Init(ctx);
        SHA384_Update(ctx, key, keyLen);
        SHA384_Final(ctx, K0);
    }
    else {
        memcpy(K0, key, keyLen);
    }
    for (size_t i = 0; i < SHA384_BLOCK_SIZE; ++i)
        K0[i] ^= 0x36;
    SHA384_Init(ctx);
    SHA384_Update(ctx, K0, SHA384_BLOCK_SIZE);
}
...
{
    uint8_t inner[SHA384_DIGEST_SIZE];
