...
#include <stddef.h>
#include <stdint.h>
#include "lsh_common.h"   /* Byte, u64, DataLength, ROL64, U8TO64_LE */

/// LSH‑512 다이제스트(출력) 크기 (바이트)
#define LSH512_DIGEST_SIZE  64
/// LSH‑512 블록 크기 (바이트)
#define LSH512_BLOCK_SIZE  256
/// LSH‑512 단계 수
#define NS512 28

/// LSH‑512 컨텍스트 구조체
typedef struct {
    int    hashbitlen;                ///< 출력 비트 길이 (항상 512)
    u64    cv512[16];                 ///< 체인 값
    Byte   Last512[LSH512_BLOCK_SIZE];///< 마지막 블록(패딩 포함)
} LSH512_CTX;

/* 내부 상수들 (SC512, IV512, gamma512) */
static const u64 SC512[NS512][8] = {
