#include <stdio.h>
#include "ec_kcdsa.h"

int KCDSA() {
	MPZ sk;
	ECC_PARAMS curve;
	ECC_POINT public_point;

	unsigned char msg[] =
	{
		0x54, 0x68, 0x69, 0x73, 0x20, 0x69, 0x73, 0x20, 0x61, 0x20, 0x73, 0x61, 0x6D, 0x70, 0x6C, 0x65,
		0x20, 0x6D, 0x65, 0x73, 0x73, 0x61, 0x67, 0x65, 0x20, 0x66, 0x6F, 0x72, 0x20, 0x45, 0x43, 0x2D,
		0x4B, 0x43, 0x44, 0x53, 0x41, 0x20, 0x69, 0x6D, 0x70, 0x6C, 0x65, 0x6D, 0x65, 0x6E, 0x74, 0x61,
		0x74, 0x69, 0x6F, 0x6E, 0x20, 0x76, 0x61, 0x6C, 0x69, 0x64, 0x61, 0x74, 0x69, 0x6F, 0x6E, 0x2E
	};
	int msgLen = 64;
	unsigned char user_provided_random_input[] =
	{
		
	};
	int user_provided_random_input_Len = 160;

	unsigned char sig[72];

	ECC_init_params(ECC_CHAR2_FIELD, &curve, SECT233k);				// SECP~~ 사용시, ECC_PRIME_FIELD로 변경할 것

	ECC_set_params(SECT233k, &curve);

	Private_Key_generator(&sk, curve, SECT233k, SHA256, user_provided_random_input, user_provided_random_input_Len);

	Public_Key_generator(&public_point, sk.dat, curve);

	EC_KCDSA_sign(public_point, curve, sk.dat, SECT233k, SHA256, msg, msgLen, sig);

	printf("R = ");
	for (int i = 0; i < 28; i++) {
		printf("%02X ", sig[i]);
	}
	printf("\n");
	printf("S = ");
	for (int i = 28; i < 56; i++) {
		printf("%02X ", sig[i]);
	}
	printf("\n");

	int ret = EC_KCDSA_verify(public_point, sig, curve, SECT233k, SHA256, msg, msgLen);
	printf("verify :");
	if (ret == 0)
		printf("Success\n");
	else
		printf("Fail\n");

	return 0;
}
