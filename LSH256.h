...
#include <stddef.h>  
#include <stdint.h>  
#include "lsh_common.h"   /* Byte, u32, DataLength, ROL32, U8TO32_LE */  

/// LSH‑256 다이제스트(출력) 크기 (바이트)  
#define LSH256_DIGEST_SIZE  32  
/// LSH‑256 블록 크기 (바이트)  
#define LSH256_BLOCK_SIZE  128  
/* 내부 상수 정의 (기존 SC256, IV256, gamma256) */  
#define NS256 26  

/// LSH‑256 컨텍스트 구조체  
typedef struct {
    int   hashbitlen;               ///< 출력 비트 길이 (항상 256)
    u32   cv256[16];                ///< 체인 값
    Byte  Last256[LSH256_BLOCK_SIZE]; ///< 마지막 블록(패딩 포함)
} LSH256_CTX;
...
/**  
* @brief LSH‑256 으로 데이터 업데이트  
* @param ctx   LSH‑256 컨텍스트  
* @param data  입력 바이트  
* @param len   입력 바이트 길이  
*/  
...
/**  
* @brief LSH‑256 최종화, 32바이트 다이제스트를 dump  
* @param ctx   LSH‑256 컨텍스트  
* @param hash  다이제스트 출력 버퍼 (LSH256_DIGEST_SIZE 바이트)  
*/  
