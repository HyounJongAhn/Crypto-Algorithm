...
void
eckcdsa_gfp(ECC_POINT Private, ECC_PARAMS SELECTED_CURVE, unsigned int* secret, unsigned int ECC_ID,
	unsigned int HASH_FUNCTION, unsigned char Message[], int MSG_len, unsigned char signature[])
{
	...
	SHA224_CTX AlgInfo_224;
	SHA256_CTX AlgInfo_256;
	...
	int count; // HASH_OUTPUT_LENGTH�� Temp_char_length�� ���̷� �߻��ϴ� r,v ������ �ذ��ϱ� ����
	...
	MAC_INIT_MPZ(r_xor_v, (unsigned int*)r_xor_v_dat, MPCONST_POS_SIG);
...
void
eckcdsa_gf2n(ECC_POINT Private, ECC_PARAMS SELECTED_CURVE, unsigned int* secret, unsigned int ECC_ID,
	unsigned int HASH_FUNCTION, unsigned char Message[], int MSG_len, unsigned char signature[])
{
	int i, added_len = 0, added_len2 = 0;
	unsigned int t, u, u2, K_Word_length;
	...
	srand((unsigned)time(NULL));
	unsigned int zzz = 0;
	for (zzz = 0; zzz < t / 8 + added_len; zzz++)
		KKEY_VAL_[zzz] = rand();
...
void Private_Key_generator(MPZ* Private_key, ECC_PARAMS SELECTED_CURVE, ECC_ID ecc_id, int HASH_FUNCTION, unsigned char* URAND_VAL, int URAND_len)
{
	...
	// XSEED = SNGF(user_provided_random_input, 224) ����
	if (HASH_FUNCTION == SHA224)
	...
	Bit_Truncation((unsigned long*)XVAL.dat, t);
