...
// AES 구조체 정의
typedef struct {
    unsigned int Nk;   // 키 길이
    unsigned int Nr;   // 라운드 수
    unsigned char key[32];  // 키
...
void AES_ShiftRows(unsigned char state[4][Nb]);
static unsigned char xtime(unsigned char x);
void AES_MixColumns(unsigned char state[4][Nb]);
...
void AES_InvSubBytes(unsigned char state[4][Nb]);
void AES_InvShiftRows(unsigned char state[4][Nb]);
void AES_InvMixColumns(unsigned char state[4][Nb]);
